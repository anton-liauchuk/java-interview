# Core
- [Какие нововведения в java 8?](#какие-нововведения-в-java-8)
- [В чем концептуальное отличие stream api от collection?](#в-чем-концептуальное-отличие-stream-api-от-collection)
- [Где хранится ссылка на объект?](#где-хранится-ссылка-на-объект)
- [Типы памяти в Java?](#типы-памяти-в-java)
- [Что такое java agent?](#что-такое-java-agent)
- [Возможные средства troubleshooting?](#возможные-средства-troubleshooting)
- [Что такое profiler?](#что-такое-profiler)
- [Что такое stop the world?](#что-такое-stop-the-world)
- [Чем отличается int, Integer, AtomicInteger?](#чем-отличается-int-integer-atomicinteger)
- [i++ для Integer, что будет с памятью?](#i-для-integer-что-будет-с-памятью)
- [Как ты относишься к использованию констант в интерфейсах?](#как-ты-относишься-к-использованию-констант-в-интерфейсах)
- [Контракт equals/hashcode?](#контракт-equalshashcode)
- [Правила переопределения equals/hashcode?](#правила-переопределения-equalshashcode)
- [Для equals/hashcode нужны одни и те же поля?](#для-equalshashcode-нужны-одни-и-те-же-поля)
- [Для чего используются вложенные классы?](#для-чего-используются-вложенные-классы)
- [Что лучше использовать interfaces или abstract classes?](#что-лучше-использовать-interfaces-или-abstract-classes)
- [Есть ли доступ к приватной переменной внешнего класса из внутреннего?](#есть-ли-доступ-к-приватной-переменной-внешнего-класса-из-внутреннего)
- [Различия между статическим внутренним и нестатическим внутренним классами?](#различия-между-статическим-внутренним-и-нестатическим-внутренним-классами)
- [Что не рекомендуется делать в параллельных стримах?](#что-не-рекомендуется-делать-в-параллельных-стримах)
- [методы у класса object?](#методы-у-класса-object)
- [что такое jmeter?](#что-такое-jmeter)
- [снять дамп памяти для приложения? как это сделать?](#снять-дамп-памяти-для-приложения-как-это-сделать)
